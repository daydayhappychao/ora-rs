...
impl Stream {
  ...
  fn print_message(writer: &mut Box<dyn Write>, frame: &str, message: &str) -> Result<()> {
  ...
  pub fn write(
    ...
    match start_time {
      None => Self::print_message(&mut writer, frame, message)?,
      Some(start_time) => {
        Self::print_message_with_duration(&mut writer, frame, message, start_time, stop_time)?
      }
    };
    Ok(())
  }
